...
$(document).ready(function() {
	// Получаем значение из spana и добавляем его в input
	var modefNum = $('#modef_num').text();
	$('.btn-set-filter').val(modefNum + ' ' + $('.btn-set-filter').val());

	// Обновляем значение в input при изменении spana
	$('#modef_num').on('DOMSubtreeModified', function() {
		var newModefNum = $(this).text();
		$('.btn-set-filter').val(newModefNum + ' ' + $('.btn-set-filter').val().split(' ').slice(1).join(' '));
	});
