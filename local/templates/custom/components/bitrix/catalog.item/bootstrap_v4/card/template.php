...
<div class="product-item">
    <?if(!empty($item["PROPERTIES"]["CML2_MANUFACTURER"]["VALUE"])):?>
        <div class="product-item-manufacturer"> <?=GetMessage("MANUFACTURER")." ".$item["PROPERTIES"]["CML2_MANUFACTURER"]["VALUE"]?> </div>
    <?endif;?>
	<? if ($itemHasDetailUrl): ?>
...
<? endif; ?>
	...
	{
        $priceIndex=0;
        for($i = 0; $i<count($arParams['PRODUCT_BLOCKS_ORDER']); $i++){
            if($arParams['PRODUCT_BLOCKS_ORDER'][$i] == 'price'){
                $priceIndex=$i;
                break;
            }
        }
        for($i = $priceIndex; $i<count($arParams['PRODUCT_BLOCKS_ORDER']); $i++){
            if($arParams['PRODUCT_BLOCKS_ORDER'][$i] == 'quantityLimit'){
                $arParams['PRODUCT_BLOCKS_ORDER'][$i] = "price";
                break;
            }
            $arParams['PRODUCT_BLOCKS_ORDER'][$i] = $arParams['PRODUCT_BLOCKS_ORDER'][$i+1];
        }
